	...
	if (0 == s_ctrl->id) {
		for (i = 0; i < 6; i++)	{
			if (strncmp((char *)(s_ctrl->sensordata->eeprom_name),
				...
				xiaomi_sdm439_hq_regiser_hw_info(XIAOMI_SDM439_HWID_MAIN_CAM, olive_rear_camera_str_buff[i][1]);
				break;
			}
		}
	} else if (1 == s_ctrl->id) {
		for (i = 0; i < 3; i++)	{
			if (strncmp((char *)(s_ctrl->sensordata->eeprom_name),
				...
				strlen(s_ctrl->sensordata->eeprom_name)) == 0) {
				xiaomi_sdm439_hq_regiser_hw_info(XIAOMI_SDM439_HWID_SUB_CAM, olive_front_camera_str_buff[i][1]);
				break;
			}
		}
	} else if (2 == s_ctrl->id) {
		for (i = 0; i < 5; i++)	{
			if (strncmp((char *)(s_ctrl->sensordata->sensor_name),
				...
				xiaomi_sdm439_hq_regiser_hw_info(XIAOMI_SDM439_HWID_MAIN_CAM_2, olive_rear_aux_camera_str_buff[i][1]);
				break;
			}
		}
	}
